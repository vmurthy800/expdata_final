function [model,time] = exp_dptime(pyra, model, removal)
...
tic;
[model] = filter_responses(model, pyra,removal);
time = toc;
...
function [model] = filter_responses(model, pyra, removal)
% model    object model
...
for level = 1:length(pyra.feat)
  ...
  %i is the filter number being processed
  for i = 1:length(r)

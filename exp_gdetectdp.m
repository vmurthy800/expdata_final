function [model,time] = exp_gdetectdp(pyra, model, removal, time)
...
%this figures out scores for terminal symbols
[model,time] = filter_responses(model, removal, pyra, time);

%this figures out scores for nonterminal symbols
% compute detection scores
L = model_sort(model);
%L is nonterminal symbols in array
for s = L
  for r = model.rules{s}
...
function [model,time] = filter_responses(model, removal, pyra,time)
% model    object model
...
for level = 1:length(pyra.feat)
  ...
  %length(r) is number of filters
  %i is the filter numbers
  for i = 1:length(r)
      tic;
      if ismember(i,removal) == 1
       r{1,i}(1,1) = -inf;
       J = r{i};
       J(J > min(J(:))) = min(J(:));
       r{i} = J;
